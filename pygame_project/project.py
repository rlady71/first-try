...
# 화면 타이틀 설정
pygame.display.set_caption("Duyun Basketball Game") # 게임 이름
...
image_path = resource_path("images") # images 폴더 위치 반환
print(image_path)
...
# 폭탄
bomb = pygame.image.load(os.path.join(image_path, "bomb.png"))
...
# 게임 종료 메시지 / Timeout(시간초과), Mission Complete(성공), Game Over (캐릭터 공에 맞음)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                ...
                weapons.append({
                    "pos_x" : character_x_pos + (character_width/2) - (weapon_width/2),
                    "pos_y" : character_y_pos - weapon_height,
                    "to_y" : weapon_speed * -1
                })
    ...
    # (수정) 무기 위치 조정
    for w in weapons:
        weapon_x_pos = w["pos_x"]
        weapon_y_pos = w["pos_y"]
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 세로 위치
        # 튕겨서 올라가는 처리
        if ball_pos_y >= screen_height /2 - ball_height:
    ...
    if bomb_y_pos > screen_height - stage_height - bomb_height:
        bomb_y_pos = 0
        bomb_x_pos = random.randint(0, screen_width - bomb_width)
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        ball_size = ball_images[ball_img_idx].get_rect().size
        ball_rect.left = ball_pos_x + ball_size[0] * 0.33125
        ball_rect.top = ball_pos_y + ball_size[1] * 0.61875
        ball_rect.size = (ball_size[0] * 0.375, ball_size[1] * 0.15)

        # 공과 무기들 충돌 처리
        for weapon_idx, weapon_val in enumerate(weapons):
            weapon_x_pos = weapon_val["pos_x"]
            weapon_y_pos = weapon_val["pos_y"]
